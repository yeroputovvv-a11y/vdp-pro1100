...
function parseExcel(book){
  ...
  const fields=[];
  for(let c=nc+1;c<width;c++)if(T(h[c]))fields.push({col:c,name:h[c],ref:r[c]||'',used:0});
  ...
  for(let rr=2;rr<m.length;rr++){
    ...
    used.push(rec);(has?variable:reserve).push(rec);
  }
...
async function analyze(file,fields){
  ...
  for(let p=1;p<=doc.numPages;p++){
    const c=await(await doc.getPage(p)).getTextContent();pages.push({p,lines:makeLines(c.items)});
  }
  const same=new Map();
  fields.forEach(f=>{const ref=T(f.ref);if(ref){const a=same.get(ref)||[];a.push(f.name);same.set(ref,a);}});
  return fields.map(f=>{
    const ref=T(f.ref);if(!ref)return{...f,count:0,loc:[],state:'empty'};
    let n=0,loc=[];
    for(const pg of pages)for(const line of pg.lines){const c=occ(line.t,ref);if(c){n+=c;loc.push({p:pg.p,c});}}
...
function templateRows(fieldCount,maxRows){
  const headers=['№'];
  for(let i=1;i<=fieldCount;i++)headers.push(i===1?'Номер изделия':i===2?'Фамилия':i===3?'Имя':i===4?'Отчество':i===5?'Звание':`Поле ${i}`);
  const ref=['ЭТАЛОН'];
  for(let i=1;i<=fieldCount;i++)ref.push(i===1?'529260704929':i===2?'Иванов':i===3?'Иван':i===4?'Иванович':i===5?'майор':'');
  const endCol=fieldCount+4;
  const endLetter=XLSX.utils.encode_col(endCol-1);
  const helperCol=endCol;
  const helperLetter=XLSX.utils.encode_col(helperCol-1);
  const rows=[['Показатель','Значение','','',...headers,'__data_flag'],['Общий тираж','', '', '',...ref,'']];
  rows[0][3]='№';
  rows[1][3]='ЭТАЛОН';
  for(let r=3;r<=maxRows+2;r++){
    const varStart='E',varEnd=endLetter;
    const anyFormula=`IF(COUNTA(${varStart}${r}:${varEnd}${r})>0,ROW()-2,"")`;
    const parts=[];
    for(let c=0;c<fieldCount;c++){const col=XLSX.utils.encode_col(4+c);parts.push(`LEN(TRIM(${col}${r}&""))>0`);}
    const dataFormula=`IF(OR(${parts.join(',')}),1,0)`;
    const row=['','','',`=${anyFormula}`];
    for(let i=0;i<fieldCount;i++)row.push('');
    row.push(`=${dataFormula}`);rows.push(row);
  }
  const ws=XLSX.utils.aoa_to_sheet(rows);
  ws['B2']={f:`COUNT(D3:D${maxRows+2})`};
  ws['B3']={f:`SUM(${helperLetter}3:${helperLetter}${maxRows+2})`};
  ws['B4']={f:'B2-B3'};
  ws['!cols']=[{wch:24},{wch:16},{wch:3},{wch:7}];
  for(let i=0;i<fieldCount;i++)ws['!cols'].push({wch:20});
  ws['!cols'].push({wch:3,hidden:true});
  const dataEnd=4+fieldCount;
  const headerCells=['A1','B1','D1'];
  for(let c=4;c<=dataEnd;c++)headerCells.push(XLSX.utils.encode_cell({r:0,c}));
  ...
  const rfill={fill:{patternType:'solid',fgColor:{rgb:'FFF2CC'}},font:{bold:true,italic:true,color:{rgb:'7C3AED'}},alignment:{horizontal:'center',vertical:'center',wrap_text:true}};
  headerCells.forEach(a=>{if(ws[a])ws[a].s=hdr;});
  for(let c=4;c<=dataEnd;c++){const a=XLSX.utils.encode_cell({r:1,c});if(ws[a])ws[a].s=rfill;}
  ws['A2'].s={font:{bold:true}};ws['A3'].s={font:{bold:true}};ws['A4'].s={font:{bold:true}};
...
function downloadTemplate(){
  try{
    ...
    XLSX.utils.book_append_sheet(wb,templateRows(count,1000),'Данные');
    const info=XLSX.utils.aoa_to_sheet([
      ['ШАБЛОН ДАННЫХ VDP PRO 1100',''],
      ['Строка 1','Названия колонок. № обязательна; переменные поля находятся справа от неё.'],
      ['Строка 2','ЭТАЛОННЫЕ значения для поиска по всему PDF.'],
      ['Строки 3+','Любое значение или пробел в переменной ячейке включает строку в тираж и даёт ей номер.'],
...
xlsxInput.addEventListener('change',async e=>{
  const f=e.target.files[0]||null;document.getElementById('xlsxName').textContent=f?f.name:'Файл не выбран';parsed=null;reset();
  if(!f){refresh();return;}
  try{parsed=parseExcel(XLSX.read(await f.arrayBuffer(),{type:'array',raw:true}));renderExcel(parsed);status.textContent=`Excel проверен: тираж ${parsed.used.length}, с переменными данными ${parsed.variable.length}, резерв ${parsed.reserve.length}.`;}
  catch(err){status.textContent='Ошибка чтения Excel: '+err.message;}refresh();
});
